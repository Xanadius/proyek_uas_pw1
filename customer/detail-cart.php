	...
	<div id="wrapper">
		...
		<div class="container">
			...
			if (isset($_GET['aksi']) == 'delete') {
				$id = $_GET['kd'];
				$cek = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id' AND sesi='$_SESSION[user_id]'");
				$data = mysqli_fetch_array($cek);
				$moq = $data['qty'];
				if (mysqli_num_rows($cek) == 0) {
					echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data tidak ditemukan.</div>';
				} else {
					$delete = mysqli_query($koneksi, "DELETE FROM cart WHERE kode='$id' AND sesi='$_SESSION[user_id]'");
					if ($delete) {
			...
			if (isset($_GET['add']) == 'add') {
				$id_add = $_GET['kd1'];
				$qty = 1;
				$cek1 = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id_add' AND sesi='$_sesi[user_id]'");
				...
				if (mysqli_num_rows($cek1) == 0) {
					...
					$add = mysqli_query($koneksi, "UPDATE cart SET qty=(qty+'$qty'), jumlah='$jum' WHERE kode='$id_add' AND sesi='$_sesi[user_id]'");
					if ($add) {
			...
			if (isset($_GET['less']) == 'less') {
				$id_less = $_GET['kd2'];
				$qty1 = 1;
				$cek2 = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id_less' AND sesi='$_sesi[user_id]'");
				...
				if (mysqli_num_rows($cek2) == 0) {
					...
					$less = mysqli_query($koneksi, "UPDATE cart SET qty=(qty-'$qty1'), jumlah='$jum1' WHERE kode='$id_less' AND sesi='$_sesi[user_id]'");
					if ($less) {
			...
			<table class="table table-hover table-condensed">
				...
				<?php
				//MENAMPILKAN detail-cart KERANJANG BELANJA//


				//mysql_select_db($database_conn, $conn);


				$sql = mysqli_query($koneksi, "SELECT * FROM cart WHERE sesi='$_SESSION[user_id]'");
				$no = 0;
				...
				?>
					...
					<tr>
						...
						<td>
							<center><a href="detail-cart.php?add=add&kd1=<?php echo $data['kode']; ?>" class="btn btn-xs btn-success">Add</a> <a href="detail-cart.php?less=less&kd2=<?php echo $data['kode']; ?>" class="btn btn-xs btn-warning">Less</a> <a href="detail-cart.php?aksi=delete&kd=<?php echo $data['kode']; ?>" class="btn btn-xs btn-danger">Delete</a></center>
						</td>
					</tr>
				...
				$sqlku = mysqli_query($koneksi, "SELECT SUM(jumlah) AS total FROM cart WHERE sesi='$_SESSION[user_id]'");
