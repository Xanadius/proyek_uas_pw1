...
if (empty($_SESSION['username'])) {
  ...
  $result = mysqli_query($koneksi, "SELECT po_terima.*, konfirmasi.jumlah, produk.kode AS kode_prod, produk.nama, produk.harga, customer.alamat, customer.no_telp, customer.nama AS nama_cus, po.status FROM po_terima
                ...
                LEFT JOIN po ON po_terima.no_po = po.no_po
                LEFT JOIN konfirmasi ON konfirmasi.no_po = po_terima.no_po
				WHERE po_terima.no_po='$_GET[kd]'") or die(mysqli_error($koneksi));
  ...
  while ($row = mysqli_fetch_array($result)) {
    ...
    $rupiah = "Rp.";
    $column_test = $column_test . $kode . "\n";
    $column_date = $column_date . $nama . "\n";
    $column_total = $column_total . $qty . "\n";
    $column_nilai = $column_nilai . $harga1 . "\n";
    ...
    $pdf->Ln();
    $pdf->Cell(80);
    $pdf->Cell(30, 10, 'Artisan Aroma Beans', 0, 0, 'C');
    $pdf->Ln();
    ...
    $pdf->SetX(5);
    $pdf->Cell(80, 8, 'No: ' . $no_po, 0, 0, 'L', 1);
    $pdf->SetX(45);
